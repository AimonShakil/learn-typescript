...
/*Polymorphism in TypeScript refers to the ability of different objects to be treated as instances of a common
superclass. This means that objects of different classes can be accessed and manipulated through a common interface,
such as a shared method name or property.

In TypeScript, polymorphism is typically achieved through method overriding, inheritance, and interfaces. 
Here's how each of these features contributes to polymorphism: */
...
class Vehicle { // This is our base class representing all vehicles
...
class Car extends Vehicle { // The Car class inherits from Vehicle
    color : string;
    ismodel : boolean = true


    constructor(speed : number, color: string , model: string) {
        super(speed, model);  // Call the constructor of the Vehicle class
        this.color = color;
        this.model = model;
    }
    // We can add methods specific to Car

       changeColor() { // defing a method which going to manipulate the underscore course count (private)
        this.color = "white"
       }                    
    honk() {
...
let myCar = new Car(60, "Black", 'Auara');
...
class Snake extends Animal {
    constructor(name : string) {
        super (name); //A derived class can also override a base class field or property. You can use the super. syntax to access base class methods.
    }
    move(meters =5) {
