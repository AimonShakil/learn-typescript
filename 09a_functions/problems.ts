...
//Callable with Properties - Call signature in an Object Type
...
function construction (fn: home) {
    console.log(fn.description + "returned" +fn(6));
}

function build (someArg : number) {
    return someArg >3;
}
...
type color = {
   new (blue : string): string;
};

function tn(pbr : color) {
    return new pbr("pink");
}
...
let squre = function (num : number) : number {
    return num * num
}
...
let generateRandomNumber = function (): number {
