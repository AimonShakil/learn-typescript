...
function addCalculator(num1, num2, operation = 'add') {
...
default to English. */
function generateWelcomeMessage(name, language = "English") {
    if (language === "English") {
        return `Welcome, ${name}!`;
    }
...
width and height. */
function rectangle(width = 2, height = 4) {
    return { width, height };
}
...
let Opr_Math = function (num_1 = 10, num_2 = 55) {
    if (num_2)
        return num_1 + num_2;
    else
        return num_1;
...
let greeter = function (name1, greeting = "Hi") {
...
let displayMessage = function (message, fontSize = 10) {
